...
/*
 *  ======== Platform.xs ========
 *  Generic platform support for MSP432 devices.
 */
...
function getExecCmd(prog, packagePath)
{
    ...
    var doNotKnow = "@$(ECHO) ti.platforms.msp432 platform package cannot "
...
{
    if (prog.build.target.$name.match(/gnu/)) {
        return ("ti/platforms/msp432/linkcmd_gnu.xdt");
    }
    else if (prog.build.target.$name.match(/iar/)) {
        return ("ti/platforms/msp432/linkcmd_iar.xdt");
    }
    else {
        return ("ti/platforms/msp432/linkcmd.xdt");
    }
}
...
function instance$meta$init(name)
{
    ...
    if (nameParams.length > maxParamsLength) {
        this.$module.$logWarning("The platform ti.platforms.msp432 accepts "
    ...
    if (this.deviceName == null) {
        this.$module.$logFatal("A device name must be specified for the "
            ...
            + "'ti.platforms.msp432:MSP432XYZ', where MSP432XYZ is the name "
