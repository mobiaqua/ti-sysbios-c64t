...
/*
 *  ======== Platform.xs ========
 *  Generic platform support for tm0sxx devices.
 */
...
function getExecCmd(prog, packagePath)
{
    ...
    var doNotKnow = "@$(ECHO) ti.platforms.tm0sxx platform package cannot "
...
{
    if (prog.build.target.$name.match(/gnu/)) {
        return ("ti/platforms/tm0sxx/linkcmd_gnu.xdt");
    }
    else if (prog.build.target.$name.match(/iar/)) {
        return ("ti/platforms/tm0sxx/linkcmd_iar.xdt");
    }
    else {
        return ("ti/platforms/tm0sxx/linkcmd.xdt");
    }
}
...
function instance$meta$init(name)
{
    ...
    if (nameParams.length > maxParamsLength) {
        this.$module.$logWarning("The platform ti.platforms.tm0sxx accepts "
    ...
    if (this.deviceName == null) {
        this.$module.$logFatal("A device name must be specified for the "
            ...
            + "'ti.platforms.tm0sxx:TM0SXYZ', where TM0SXYZ is the name "
    ...
    if (xdc.om.$name == "cfg") {
        ...
        this.CPU.deviceName = "TM0Sxx";
        this.CPU.clockRate = 12;
    }
}
/*
 */
