...
function genElfSymbols(prog)
{
    ...
    if (prog.build.target.isa.match(/430/)) {
        res += "--symbol_map " + prog.build.target.asmName('__TI_cleanup_ptr') 
            + "=" + prog.build.target.asmName('_cleanup_ptr') + "\n";

        res += "--symbol_map " + prog.build.target.asmName('__TI_dtors_ptr') 
            + "=" + prog.build.target.asmName('_dtors_ptr') + "\n";
    }
