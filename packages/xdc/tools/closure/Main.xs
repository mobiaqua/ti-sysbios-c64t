...
function run(cmdr, args)
{
    ...
    for each (item in cfgxml.imports["package"]) {
        ...
        pkgBase[counter] = _unixJSPath(item.@repository);
    ...
    /* 'counter' may have gone above the length of arrays in the loop above,
     * and the array sizes are adjusted here so that we can use 'length' in
     * the further loops.
     */
    pkgName.length = counter;
    pkgBase.length = counter;
    ...
    /*
     * The file rov.xs is always copied next to the .out file and used for ROV.
     */
...
function _deleteMisc(dir, agg, base, libs) {
    ...
    for (var i = 0; i < files.length; i++) {
        ...
        if (f.isDirectory()) {
            ...
            if (!files[i].match(/.*\.h$/) && !files[i].match(/.*\.rov\.xs$/)) {
                var neededLib = false;
